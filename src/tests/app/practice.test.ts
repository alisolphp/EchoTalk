...
const waitFor = async (predicate: () => boolean, timeout = 2000) => {
    ...
    while (Date.now() - start < timeout) {
        if (predicate()) return;
        // اجازه بده چرخه رویداد و میکروتسک‌ها اجرا شوند
        await sleep(0);
    }
...
describe('Practice Logic', () => {
    ...
    beforeEach(() => {
        // Ensure real timers inside this test too (safety)
        vi.useRealTimers();

        // Mock $.getJSON to prevent real XHR requests
    ...
    it('should finish session and show final message when finishSession is invoked', async () => {
        ...
        // Directly invoke private method to skip full flow
    ...
    it('should start and stop recording when audio recording is enabled', async () => {
        // Enable recording before init so loadState picks it up
        localStorage.setItem('shadow_record_audio', 'true');

        await app.init();
        $('#startBtn').trigger('click');

        // منتظر بمون تا زنجیره onend → startRecording → getUserMedia کامل بشه
        ...
        // Simulate user manually starting and stopping recording
        mr.start();
        mr.stop();
